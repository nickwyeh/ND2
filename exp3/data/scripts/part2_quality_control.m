...
% control checks for Experiment 3. The participant log is also updated . This follows the pre-registered
% criteria found on OSF (*add in new OSF website when completed*)for Experiment 3. 
% This script will take the raw data folder on OSF as input and output data
% to the data analysis folder.
%These include:
...
bad_data_reason = cell2table(  cell(0,9),...
'VariableNames', {'participant' 'status' 'study_cb' 'list' 'study' 'test' 'confidence' 'item' 'source'}); 
...
for pari = 1:size(par_log,1)
    ...
    if mem_perf_good & ~isnan(item_pr)
        fprintf('(GOOD)\n');
        item_status = 0;
    else
        fprintf('(BAD)\n');
    ...
    if source_perf_good & ~isnan(source_pr)
        fprintf('(GOOD)\n');
        source_status = 0;
    else
        fprintf('(BAD)\n');
    ...
    if conf_resp_rate_good & ~isnan(pNot6or1)
        fprintf('(GOOD)\n');
        confidence_status = 0;
    else
        fprintf('(BAD)\n');
    ...
    writetable(test_data, combo_beh_file, 'FileType','text','Delimiter','\t');
        ...
        %% Store bad data reason
    bad_data_reason = vertcat(bad_data_reason, ...
        {id participant_status study_cb list study_status test_status confidence_status item_status source_status });
end
%% Step 10: Save the combined participant log to data files folder for part3_analyze.m script
par_tsv_file = fullfile(directories.data_files,'participants.tsv');
writetable(par_log,par_tsv_file,'FileType','text','Delimiter','\t');
writetable(bad_data_reason,fullfile(directories.analyses,'bad_data_reason.csv'));
