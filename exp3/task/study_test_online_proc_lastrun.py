...
expName = 'study_test_online_proc'  # from the Builder filename that created this script
expInfo = {'id': ''}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='X:\\EXPT\\nd002\\exp3\\task\\study_test_online_proc_lastrun.py',
...
# Initialize components for Routine "uncued_routine"
uncued_routineClock = core.Clock()
uncued_fixation_text = visual.TextStim(win=win, name='uncued_fixation_text',
    text='+',
    font='Arial',
...
# -------Start Routine "experiment_setup"-------
...
study_instruction_loop = data.TrialHandler(nReps=1, method='sequential', 
...
for thisStudy_instruction_loop in study_instruction_loop:
    ...
    while continueRoutine:
        ...
        if frameN >= 0.0 and instruction_image.status == NOT_STARTED:
            # keep track of start time/frame for later
            instruction_image.tStart = t  # not accounting for scr refresh
        ...
        if frameN >= 0.0 and practice_key_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
            practice_key_resp.tStart = t  # not accounting for scr refresh
            ...
            practice_key_resp.clearEvents(eventType='keyboard')
    ...
    routineTimer.reset()
# completed 1 repeats of 'study_instruction_loop'


# set up handler to look after randomisation of conditions etc
confirm_loop = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
...
for thisConfirm_loop in confirm_loop:
    ...
    t = 0
    confirm_question_routineClock.reset()  # clock
    frameN = -1
    continueRoutine = True
    # update component parameters for each repeat
    ...
    # -------Start Routine "confirm_question_routine"-------
    while continueRoutine:
        # get current time
        ...
        if frameN >= 0.0 and confirm_question_image.status == NOT_STARTED:
            # keep track of start time/frame for later
            confirm_question_image.tStart = t  # not accounting for scr refresh
        ...
        if frameN >= 0.0 and confirm_question_key_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
            confirm_question_key_resp.tStart = t  # not accounting for scr refresh
            ...
            confirm_question_key_resp.clearEvents(eventType='keyboard')
        if confirm_question_key_resp.status == STARTED:
    ...
    t = 0
    confirm_answer_routineClock.reset()  # clock
    frameN = -1
    continueRoutine = True
    # update component parameters for each repeat
    ...
    # -------Start Routine "confirm_answer_routine"-------
    while continueRoutine:
        # get current time
        ...
        if frameN >= 0.0 and confirm_answer_image.status == NOT_STARTED:
            # keep track of start time/frame for later
            confirm_answer_image.tStart = t  # not accounting for scr refresh
        ...
        if frameN >= 0.0 and confirm_answer_text.status == NOT_STARTED:
            # keep track of start time/frame for later
            confirm_answer_text.tStart = t  # not accounting for scr refresh
        ...
        if frameN >= 0.0 and confirm_answer_key_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
            confirm_answer_key_resp.tStart = t  # not accounting for scr refresh
            ...
            confirm_answer_key_resp.clearEvents(eventType='keyboard')
        if confirm_answer_key_resp.status == STARTED:
    ...
    routineTimer.reset()
# completed 1 repeats of 'confirm_loop'


# set up handler to look after randomisation of conditions etc
study_overview_loop = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
...
for thisStudy_overview_loop in study_overview_loop:
    ...
    t = 0
    study_overview_instructionsClock.reset()  # clock
    frameN = -1
    continueRoutine = True
    # update component parameters for each repeat
    ...
    # -------Start Routine "study_overview_instructions"-------
    while continueRoutine:
        # get current time
        ...
        if frameN >= 0.0 and overview_instructions_image.status == NOT_STARTED:
            # keep track of start time/frame for later
            overview_instructions_image.tStart = t  # not accounting for scr refresh
        ...
        if t >= 0.0 and study_warning_key_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
            study_warning_key_resp.tStart = t  # not accounting for scr refresh
            ...
            study_warning_key_resp.clearEvents(eventType='keyboard')
        if study_warning_key_resp.status == STARTED:
    ...
    routineTimer.reset()
# completed 1 repeats of 'study_overview_loop'


# set up handler to look after randomisation of conditions etc
phases = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
...
for thisPhase in phases:
    ...
    elif phase_name == 'uncued_prac':
        instruction_img_file = 'uncued_prac_instructions.png'
    elif phase_name == 'informed1_study1' or phase_name == 'informed2_study1' or phase_name == 'informed1_study2' or phase_name == 'informed2_study2' or phase_name == 'informed1_study3' or phase_name == 'informed2_study3' or phase_name == 'informed1_study4' or phase_name == 'informed2_study4':
        instruction_img_file = 'informed_real_instructions.png'
    elif phase_name == 'uninformed1_study1' or phase_name == 'uninformed2_study1' or phase_name == 'uninformed1_study2' or phase_name == 'uninformed2_study2' or phase_name == 'uninformed1_study3' or phase_name == 'uninformed2_study3' or phase_name == 'uninformed1_study4' or phase_name == 'uninformed2_study4':
        instruction_img_file = 'uninformed_real_instructions.png'
    else :
        instruction_img_file = 'uncued_real_instructions.png'
    # keep track of which components have finished
    ...
    while continueRoutine:
        ...
        if t >= 0.0 and instructions_image.status == NOT_STARTED:
            # keep track of start time/frame for later
            instructions_image.tStart = t  # not accounting for scr refresh
        ...
        if frameN >= 0.0 and study_phase_progress_text.status == NOT_STARTED:
            # keep track of start time/frame for later
            study_phase_progress_text.tStart = t  # not accounting for scr refresh
        ...
        if t >= 0.0 and instructions_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
            instructions_resp.tStart = t  # not accounting for scr refresh
            ...
            # keyboard checking is just starting
            instructions_resp.clearEvents(eventType='keyboard')
    ...
    t = 0
    pause_routineClock.reset()  # clock
    frameN = -1
    continueRoutine = True
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if t >= 0.0 and pause_text.status == NOT_STARTED:
            # keep track of start time/frame for later
            pause_text.tStart = t  # not accounting for scr refresh
        ...
        if pause_text.status == STARTED and t >= frameRemains:
    ...
    for thisTrial in trials:
        ...
        for thisVis_cue_loop in vis_cue_loop:
            ...
            t = 0
            visual_cue_routineClock.reset()  # clock
            frameN = -1
            continueRoutine = True
            # update component parameters for each repeat
            ...
            # -------Start Routine "visual_cue_routine"-------
            while continueRoutine:
                # get current time
                ...
                if frameN >= 0 and visual_cue_rect.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    visual_cue_rect.tStart = t  # not accounting for scr refresh
                ...
                if visual_cue_rect.status == STARTED and frameN >= (visual_cue_rect.frameNStart + vis_cue_dur):
                    # keep track of stop time/frame for later
                ...
                if frameN >= vis_cue_dur and visual_cue_fixation.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    visual_cue_fixation.tStart = t  # not accounting for scr refresh
                ...
                if visual_cue_fixation.status == STARTED and frameN >= (visual_cue_fixation.frameNStart + vis_cue_fix_dur):
                    # keep track of stop time/frame for later
        ...
        # set up handler to look after randomisation of conditions etc
        vis_uncued_loop = data.TrialHandler(nReps=show_nocue, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=[None],
            seed=None, name='vis_uncued_loop')
        thisExp.addLoop(vis_uncued_loop)  # add the loop to the experiment
        thisVis_uncued_loop = vis_uncued_loop.trialList[0]  # so we can initialise stimuli with some values
        # abbreviate parameter names if possible (e.g. rgb = thisVis_uncued_loop.rgb)
        if thisVis_uncued_loop != None:
            for paramName in thisVis_uncued_loop:
                exec('{} = thisVis_uncued_loop[paramName]'.format(paramName))
        
        for thisVis_uncued_loop in vis_uncued_loop:
            currentLoop = vis_uncued_loop
            # abbreviate parameter names if possible (e.g. rgb = thisVis_uncued_loop.rgb)
            if thisVis_uncued_loop != None:
                for paramName in thisVis_uncued_loop:
                    exec('{} = thisVis_uncued_loop[paramName]'.format(paramName))
            
            # ------Prepare to start Routine "uncued_routine"-------
            t = 0
            uncued_routineClock.reset()  # clock
            frameN = -1
            continueRoutine = True
            # update component parameters for each repeat
            # keep track of which components have finished
            uncued_routineComponents = [uncued_fixation_text]
            for thisComponent in uncued_routineComponents:
                thisComponent.tStart = None
                thisComponent.tStop = None
                thisComponent.tStartRefresh = None
                thisComponent.tStopRefresh = None
            ...
            # -------Start Routine "uncued_routine"-------
            while continueRoutine:
                # get current time
                t = uncued_routineClock.getTime()
                frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
                # update/draw components on each frame
                
                # *uncued_fixation_text* updates
                if frameN >= 0.0 and uncued_fixation_text.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    uncued_fixation_text.tStart = t  # not accounting for scr refresh
                    uncued_fixation_text.frameNStart = frameN  # exact frame index
                    win.timeOnFlip(uncued_fixation_text, 'tStartRefresh')  # time at next scr refresh
                    uncued_fixation_text.setAutoDraw(True)
                if uncued_fixation_text.status == STARTED and frameN >= (uncued_fixation_text.frameNStart + vis_cue_dur + vis_cue_fix_dur):
                    # keep track of stop time/frame for later
                    uncued_fixation_text.tStop = t  # not accounting for scr refresh
                    uncued_fixation_text.frameNStop = frameN  # exact frame index
                    win.timeOnFlip(uncued_fixation_text, 'tStopRefresh')  # time at next scr refresh
                    uncued_fixation_text.setAutoDraw(False)
            ...
            # -------Ending Routine "uncued_routine"-------
            for thisComponent in uncued_routineComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            vis_uncued_loop.addData('uncued_fixation_text.started', uncued_fixation_text.tStartRefresh)
            vis_uncued_loop.addData('uncued_fixation_text.stopped', uncued_fixation_text.tStopRefresh)
            # the Routine "uncued_routine" was not non-slip safe, so reset the non-slip timer
            routineTimer.reset()
        # completed show_nocue repeats of 'vis_uncued_loop'
        
        
        # ------Prepare to start Routine "visual_word_routine"-------
        t = 0
        visual_word_routineClock.reset()  # clock
        frameN = -1
        continueRoutine = True
        # update component parameters for each repeat
        ...
        # -------Start Routine "visual_word_routine"-------
        while continueRoutine:
            # get current time
            ...
            if frameN >= 0 and uninformed_cue_box.status == NOT_STARTED:
                # keep track of start time/frame for later
                uninformed_cue_box.tStart = t  # not accounting for scr refresh
            ...
            if uninformed_cue_box.status == STARTED and frameN >= (uninformed_cue_box.frameNStart + vis_word_dur):
                # keep track of stop time/frame for later
            ...
            if frameN >= 0 and visual_word_text.status == NOT_STARTED:
                # keep track of start time/frame for later
                visual_word_text.tStart = t  # not accounting for scr refresh
            ...
            if visual_word_text.status == STARTED and frameN >= (visual_word_text.frameNStart + vis_word_dur):
                # keep track of stop time/frame for later
            ...
            if frameN >= vis_word_dur and visual_word_fixation.status == NOT_STARTED:
                # keep track of start time/frame for later
                visual_word_fixation.tStart = t  # not accounting for scr refresh
            ...
            if visual_word_fixation.status == STARTED and frameN >= (visual_word_fixation.frameNStart + vis_word_fix_dur):
                # keep track of stop time/frame for later
            ...
            # *visual_resp* updates
            if frameN >= 0 and visual_resp.status == NOT_STARTED:
                # keep track of start time/frame for later
                visual_resp.tStart = t  # not accounting for scr refresh
                ...
                visual_resp.clearEvents(eventType='keyboard')
            if visual_resp.status == STARTED and frameN >= (visual_resp.frameNStart + vis_word_dur + vis_word_fix_dur):
                # keep track of stop time/frame for later
        ...
        t = 0
        code_study_respClock.reset()  # clock
        frameN = -1
        continueRoutine = True
        # update component parameters for each repeat
        ...
        if study_resp_key == 'j'or study_resp_key == 'k':
    ...
    phases.addData('pause_text.started', pause_text.tStartRefresh)
    phases.addData('pause_text.stopped', pause_text.tStopRefresh)
# completed 1 repeats of 'phases'


# ------Prepare to start Routine "study_complete"-------
t = 0
study_completeClock.reset()  # clock
frameN = -1
continueRoutine = True
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if t >= 0.0 and study_complete_text.status == NOT_STARTED:
        # keep track of start time/frame for later
        study_complete_text.tStart = t  # not accounting for scr refresh
    ...
    if study_complete_text.status == STARTED and t >= frameRemains:
        # keep track of stop time/frame for later
...
for thisTest_phase in test_phase:
    ...
    t = 0
    test_phase_setupClock.reset()  # clock
    frameN = -1
    continueRoutine = True
    # update component parameters for each repeat
    ...
    if phase_name == '_prac':
        ...
        test_task = 'test'
        select_test = study_test[stimuli_cb_options[0]]
        stim_file_test = 'test'+ select_test +'.csv'
        print(stim_file_test)
    # keep track of which components have finished
    ...
    # -------Start Routine "test_phase_setup"-------
    while continueRoutine:
        # get current time
    ...
    while continueRoutine:
        ...
        if frameN >= 0.0 and test_instructions_image.status == NOT_STARTED:
            # keep track of start time/frame for later
            test_instructions_image.tStart = t  # not accounting for scr refresh
        ...
        if t >= 0.0 and test_instr_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
            test_instr_resp.tStart = t  # not accounting for scr refresh
            ...
            # keyboard checking is just starting
            test_instr_resp.clearEvents(eventType='keyboard')
        if test_instr_resp.status == STARTED:
    ...
    t = 0
    warn_prac_or_critClock.reset()  # clock
    frameN = -1
    continueRoutine = True
    # update component parameters for each repeat
    ...
    # -------Start Routine "warn_prac_or_crit"-------
    while continueRoutine:
        # get current time
        ...
        if frameN >= 0.0 and test_warning_text.status == NOT_STARTED:
            # keep track of start time/frame for later
            test_warning_text.tStart = t  # not accounting for scr refresh
        ...
        if frameN >= 0.0 and warning_key_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
            warning_key_resp.tStart = t  # not accounting for scr refresh
            ...
            # keyboard checking is just starting
            warning_key_resp.clearEvents(eventType='keyboard')
        if warning_key_resp.status == STARTED:
    ...
    for thisTest_trial in test_trials:
        ...
        t = 0
        selec_test_breakClock.reset()  # clock
        frameN = -1
        continueRoutine = True
        # update component parameters for each repeat
        if break_count == 89 :
        ...
        # -------Start Routine "selec_test_break"-------
        while continueRoutine:
            # get current time
        ...
        for thisTest_break_loop in test_break_loop:
            ...
            while continueRoutine:
                ...
                if frameN >= 0.0 and draw_test_break.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    draw_test_break.tStart = t  # not accounting for scr refresh
                ...
                if frameN >= 0.0 and draw_counter_text.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    draw_counter_text.tStart = t  # not accounting for scr refresh
                ...
                if frameN >= 0.0 and draw_test_break_key_resp.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    draw_test_break_key_resp.tStart = t  # not accounting for scr refresh
                    ...
                    # keyboard checking is just starting
                    draw_test_break_key_resp.clearEvents(eventType='keyboard')
                if draw_test_break_key_resp.status == STARTED:
        ...
        while continueRoutine:
            ...
            if frameN >= 0.0 and test_delay_text.status == NOT_STARTED:
                # keep track of start time/frame for later
                test_delay_text.tStart = t  # not accounting for scr refresh
            ...
            if test_delay_text.status == STARTED and frameN >= (test_delay_text.frameNStart + test_delay_frames):
                # keep track of stop time/frame for later
        ...
        while continueRoutine:
            ...
            if frameN >= 0.0 and item_rating_text.status == NOT_STARTED:
                # keep track of start time/frame for later
                item_rating_text.tStart = t  # not accounting for scr refresh
            ...
            if item_rating_text.status == STARTED and frameN >= test_maxtime_frames:
                # keep track of stop time/frame for later
            ...
            # *item_slider* updates
            if frameN >= 0.0 and item_slider.status == NOT_STARTED:
                # keep track of start time/frame for later
                item_slider.tStart = t  # not accounting for scr refresh
            ...
            if item_slider.status == STARTED and frameN >= test_maxtime_frames:
                # keep track of stop time/frame for later
        ...
        while continueRoutine:
            ...
            # *item_text* updates
            if frameN >= 0.0 and item_text.status == NOT_STARTED:
                # keep track of start time/frame for later
                item_text.tStart = t  # not accounting for scr refresh
            ...
            if item_text.status == STARTED and frameN >= (item_text.frameNStart + test_maxtime_frames):
                # keep track of stop time/frame for later
            ...
            # *sm_slider* updates
            if frameN >= 0.0 and sm_slider.status == NOT_STARTED:
                # keep track of start time/frame for later
                sm_slider.tStart = t  # not accounting for scr refresh
            ...
            if sm_slider.status == STARTED and frameN >= (sm_slider.frameNStart + test_maxtime_frames):
                # keep track of stop time/frame for later
        ...
        while continueRoutine:
            ...
            if frameN >= 0.0 and test_iti_text.status == NOT_STARTED:
                # keep track of start time/frame for later
                test_iti_text.tStart = t  # not accounting for scr refresh
            ...
            if test_iti_text.status == STARTED and frameN >= (test_iti_text.frameNStart + test_iti_frames):
                # keep track of stop time/frame for later
        ...
        t = 0
        code_test_respClock.reset()  # clock
        frameN = -1
        continueRoutine = True
        # update component parameters for each repeat
        # Extract the appropriate response from the correct object
        ...
        test_trials.addData('test_cb',stim_file_test)
        # keep track of which components have finished
...
t = 0
test_completeClock.reset()  # clock
frameN = -1
continueRoutine = True
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if t >= 0.0 and test_complete_text.status == NOT_STARTED:
        # keep track of start time/frame for later
        test_complete_text.tStart = t  # not accounting for scr refresh
    ...
    if test_complete_text.status == STARTED and t >= frameRemains:
        # keep track of stop time/frame for later
