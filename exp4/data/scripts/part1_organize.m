%% Organize  Data for Experiment 4 (for additional details see OSF, https://osf.io/y9h2c)

% Overview: This script will organize study and test data from Experiment 4 and clean up variable names
% and responses and output 4 files. 1) Study datat, 2) test data, 3) study_test data, and 4) Update participant log. 

% Requirements to run this script: 
% 1) Source data in the source folder.
% 2) Participant log in source folder
% 3) Matlab toolbox from https://github.com/koenlab/koenlab_tools
...
for pari = 1:length(participant_list)
    ...
    vars_to_remove = {'frameRate' 'date' 'expName' 'ran' 'order' 'visual_word_text_started' 'visual_word_text_stopped' ...
        ...
        'test_trials_ran' 'test_trials_order' 'study_warning_key_resp_keys' 'study_warning_key_resp_rt' 'study_phase'};
    ...
    source_study_test_file = fullfile( directories.par_source, sprintf('PARTICIPANT_study_test_online_proc_%s.csv', participant) ); 
    study_test_opts = detectImportOptions( source_study_test_file, 'FileType', 'text' );
    study_test_data = readtable( source_study_test_file, study_test_opts );
    study_test_data = study_test_data( :, ~ismember(study_test_data.Properties.VariableNames, vars_to_remove) );
    end
    ...
    study_variables_to_keep = {'id' 'psychopyVersion' 'phase_name' ...
        ...
        'study_resp_key' 'study_resp' 'study_rt' 'study_iti'};
    
    % Define the rows to keep in the data
    study_rows_to_keep = contains(study_test_data.phase_name,'real');
    ...
    % Code for list 
    study_list = study_data.phase_name{1};
    list = study_list(end); % The last character is the list. Note it is a number coded as a string
    
    % Add stimuli list
    study_data.list(:)     = list;
    
    % Move them in the data table
    study_data = movevars(study_data,{'list'},'After','id');
    
    % Update ID in  data
    study_data.id(:) = {id}; 

    % Create study_judgment accuracy variable based on reviewers comments.
    cap_no = {'N','n'};
    cap_yes = {'Y','y'};
    ...
    % Move them in the data table
    test_data = movevars(test_data,{'list'},'After','id');
    ...
    test_data.study_iti        = -99*ones(size(test_data,1),1); % Initialize as all missing dat
    ...
    for trli = 1:size(test_data,1)
        ...
        if ismember(test_data.old_new(trli),'new')
            ...
            test_data.study_iti(trli)        = study_data.study_iti(study_idx);
...
demo_data_file = fullfile( directories.source, sprintf('p2e4_demographics.csv') );
...
final_variables_to_keep ={'OG_id' 'id' 'list' 'Age' 'Gender' 'Ethnicity'};
