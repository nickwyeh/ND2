...
for pari = 1:length(participant_list)
 ...
 %% Check study judgment accuracy (reviewer comments)
    ...
    %% Add in stay/switch trial information 
    ...
    study_rt_data = vertcat(study_rt_data, ...
        { participant status  list 'informed'  'SC'   median(study_rts(informed & old_trial & old_resp & source_cor & good_trial)) }, ...
        { participant status  list 'informed'  'SIDK' median(study_rts(informed & old_trial & old_resp & source_miss & good_trial)) }, ...
        { participant status  list 'informed'  'Miss' median(study_rts(informed & old_trial & ~old_resp & good_trial)) }, ...
        { participant status  list 'uninformed'  'SC'   median(study_rts(uninformed & old_trial & old_resp & source_cor & good_trial)) }, ...
        { participant status  list 'uninformed'  'SIDK' median(study_rts(uninformed & old_trial & old_resp & source_miss & good_trial)) }, ...
        { participant status  list 'uninformed'  'Miss' median(study_rts(uninformed & old_trial & ~old_resp & good_trial)) },...
        { participant status  list 'uncued'  'SC'   median(study_rts(uncued & old_trial & old_resp & source_cor & good_trial)) }, ...
        { participant status  list 'uncued'  'SIDK' median(study_rts(uncued & old_trial & old_resp & source_miss & good_trial)) }, ...
        { participant status  list 'uncued'  'Miss' median(study_rts(uncued & old_trial & ~old_resp  & good_trial)) } );
...
writetable(test_rt_data_wide,fullfile(directories.analyses,'testrt_wide.csv'));
