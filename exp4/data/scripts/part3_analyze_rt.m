%% Data Analysis for Experiment 4 (for additional details see OSF, https://osf.io/y9h2c)
% Overview: This script will take input from the data analyis folder and output excel
% files for memory, study RT's, and ROCs for Experiment 4. 

% Required to run this script:
% 1) Run the part 1 and part 2 scripts
% 2) Data and participant log in the data analysis folder (output from part 2 script)
% 3) The ROC toolbox will be requiedto compute recollection and familiarity estimates. 
% For additional information on ROC tool box see, https://pubmed.ncbi.nlm.nih.gov/27573007/
% 4) Matlab toolbox from https://github.com/koenlab/koenlab_tools
...
elseif ismac
    directories.top    = '/Volumes/koendata/EXPT/nd002/exp4/data/';
elseif isunix
    directories.top    = '/koenlab/koendata/EXPT/nd002/exp4/data/';
end
...
par_log_opts = detectImportOptions(par_log_file, 'FileType', 'text','Delimiter','\t' );
...
hit_fa_data = cell2table(  cell(0,5),...
'VariableNames', {'participant' 'status'  'list' 'study_condition' 'proportion'});

memory_acc_data = cell2table(  cell(0,12),...
    'VariableNames', {'participant' 'status'  'list'  'study_condition' 'auc' 'corrected_recognition' 'Ro' 'F'  'corrected_source' 'sourcePr' 'source_hit' 'source_dk'});

study_rt_data   = cell2table(  cell(0,6),...
    'VariableNames',{'participant' 'status'  'list'  'study_condition' 'subsequent_memory' 'median_rt'});

study_acc_data = cell2table( cell(0,6),...
'VariableNames', {'participant' 'status'  'list' 'study_condition' 'proportion' 'study_iti'});

test_rt_data = cell2table(  cell(0,6),...
    'VariableNames',{'participant' 'status'  'list'  'study_condition' 'item_rt' 'source_rt'});

pivot_table = array2table(zeros(0,25), 'VariableNames',{'id','list','test_cb','word','old_new',...
    ...
    'manmade','shoebox','study_judgment_correct_resp','good_trial','status'});
...
for pari = 1:length(participant_list)
    ...
    c = 1; % counter
     for i = 6:-1:1
            ...
            targf(2,c) = sum( test_data.item_resp(old_trial & uninformed & good_trial & good_rt_trial) == i );
            targf(3,c) = sum( test_data.item_resp(old_trial & uncued & good_trial & good_rt_trial) == i );
   ...
   % save(fullfile(directories.par_analysis,'roc_data.mat'),'roc_data');
    ...
    % Uninformed
    p_shit_uninformed   = mean(source_cor(old_trial & old_resp & uninformed & good_trial & good_rt_trial));
    p_sdk_uninformed    = mean(source_dk(old_trial & old_resp & uninformed & good_trial & good_rt_trial));
    uninformed_sourcePR = (p_shit_uninformed - .5*(1-p_sdk_uninformed)) / (1 - .5*(1-p_sdk_uninformed));
    % Uncued
    p_shit_uncued       = mean(source_cor(old_trial & old_resp & uncued & good_trial & good_rt_trial));
    p_sdk_uncued    = mean(source_dk(old_trial & old_resp & uncued & good_trial & good_rt_trial));
    ...
    source_fa_uncued        = mean(source_inc(old_trial & old_resp & uncued & good_trial & good_rt_trial));
    %hits - FA
 ...
 %% Check study judgment accuracy (reviewer comments)
    ...
    % Grab study ITI
    study_iti_informed = mean(test_data.study_iti(old_trial & informed & good_trial));
    study_iti_uninformed = mean(test_data.study_iti(old_trial & uninformed & good_trial));
    study_iti_uncued = mean(test_data.study_iti(old_trial & uncued & good_trial));
    
    study_acc_data =  vertcat(study_acc_data, ...
        {participant status  list 'informed' informed_study study_iti_informed}, ...
        {participant status  list 'uninformed' uninformed_study study_iti_uninformed}, ...
        {participant status  list 'uncued' uncued_study study_iti_uncued});
    %% Store hit and FA data
    hit_fa_data = vertcat(hit_fa_data, ...
        {participant status  list 'informed' informed_hits}, ...
        {participant status  list 'uninformed' uninformed_hits}, ...
        {participant status  list 'uncued' uncued_hits}, ...
        {participant status  list 'new' false_alarms });
   
     memory_acc_data = vertcat(memory_acc_data, ...
        {participant status  list 'informed' informed_auc informed_cor_recog informed_Ro informed_F   cr_s_informed informed_sourcePR p_shit_informed p_sdk_informed}, ...
        {participant status  list 'uninformed' uninformed_auc uninformed_cor_recog uninformed_Ro uninformed_F   cr_s_uninformed uninformed_sourcePR p_shit_uninformed p_sdk_uninformed},...
        {participant status  list 'uncued' uncued_auc uncued_cor_recog uncued_Ro uncued_F   cr_s_uncued uncued_sourcePR p_shit_uncued p_sdk_uncued} );
    ...
    % source correct (SC), source dont know (SIDK), miss (item miss) 
    study_rt_data = vertcat(study_rt_data, ...
        { participant status  list 'informed'  'SC'   median(study_rts(informed & old_trial & old_resp & source_cor & good_trial & good_rt_trial)) }, ...
        { participant status  list 'informed'  'SIDK' median(study_rts(informed & old_trial & old_resp & source_miss & good_trial & good_rt_trial)) }, ...
        { participant status  list 'informed'  'Miss' median(study_rts(informed & old_trial & ~old_resp & good_trial & good_rt_trial)) }, ...
        { participant status  list 'uninformed'  'SC'   median(study_rts(uninformed & old_trial & old_resp & source_cor & good_trial & good_rt_trial)) }, ...
        { participant status  list 'uninformed'  'SIDK' median(study_rts(uninformed & old_trial & old_resp & source_miss & good_trial & good_rt_trial)) }, ...
        { participant status  list 'uninformed'  'Miss' median(study_rts(uninformed & old_trial & ~old_resp & good_trial & good_rt_trial)) },...
        { participant status  list 'uncued'  'SC'   median(study_rts(uncued & old_trial & old_resp & source_cor & good_trial & good_rt_trial)) }, ...
        { participant status  list 'uncued'  'SIDK' median(study_rts(uncued & old_trial & old_resp & source_miss & good_trial & good_rt_trial)) }, ...
        { participant status  list 'uncued'  'Miss' median(study_rts(uncued & old_trial & ~old_resp  & good_trial & good_rt_trial)) } );
    ...
    %% Step 5b check test RTs

         % Gather trials of use
    item_rts = test_data.item_rt; % Get out item RTs now
    sm_rts = test_data.sm_rt; % Get out source RTs now
    test_good = test_data.test_good_trial;

    % Get a median RT and store in study_rt_data
    % source correct (SC), source dont know (SIDK), miss (item miss) 
    test_rt_data = vertcat(test_rt_data, ...
        { participant status  list 'informed'     mean(item_rts(informed & old_trial  & test_good)) mean(sm_rts(informed & old_trial & old_resp & test_good)) }, ...
        { participant status  list 'uninformed'    mean(item_rts(uninformed & old_trial  & test_good)) mean(sm_rts(uninformed & old_trial  & old_resp & test_good)) }, ...
        { participant status  list 'uncued'   mean(item_rts(uncued & old_trial & test_good)) mean(sm_rts(uncued & old_trial  & old_resp & test_good)) });
   ...
   %% Step 6 collect aggregate data for ROC
    ...
    temp_luref = [];
     ...
     for i = 6:-1:1
            ...
            temp_targf(2,c) = sum( test_data.item_resp(old_trial & uninformed & good_trial & good_rt_trial) == i ) ;
            meta_targf(2,c) = temp_targf(2,c)+ meta_targf(2,c);
            
            temp_targf(3,c) = sum( test_data.item_resp(old_trial & uncued & good_trial & good_rt_trial) == i ) ;
...
%% Step 7 Make each cell array a data table (in wide and long formats) and save data
study_acc_data_wide = unstack(study_acc_data,{'proportion' 'study_iti'},'study_condition');
study_acc_data_wide = movevars(study_acc_data_wide,{'proportion_uncued'},'After','proportion_uninformed');
study_acc_data_wide = movevars(study_acc_data_wide,{'study_iti_uncued'},'After','study_iti_uninformed');
...
% test RT data
test_rt_data_wide =  unstack(test_rt_data,{'item_rt' 'source_rt'},'study_condition');
...
writetable(memory_acc_data,fullfile(directories.analyses,'rt_memory_measures_data_long.csv'));
...
writetable(test_rt_data_wide,fullfile(directories.analyses,'rt_testrt_wide.csv'));

writetable(pivot_table,fullfile(directories.analyses,'rt_pivot_table.csv'));
