...
pivot_table = array2table(zeros(0,27),...
    'VariableNames',{'id','study_cb','list','test_cb','word','old_new',...
    'study_judgment','cue_condition','item_resp','item_rt','item_acc','old_new_resp','sm_resp','sm_rt','sm_acc',...
    'test_good_trial','study_resp','study_rt','study_good_trial','manmade','shoebox','study_judgment_correct_resp',...
    'stayswitch','stayswitch_run','good_trial','status','trial'});
...
for pari = 1:length(participant_list)
    ...
    c = 1; % counter
     for i = 6:-1:1
            ...
            targf(2,c) = sum( test_data.item_resp(old_trial & ~informed & good_trial) == i );
    ...
    p_shit_uninformed = mean(source_cor(old_trial & old_resp & ~informed & good_trial));
    p_sdk_uninformed  = mean(source_dk(old_trial & old_resp & ~informed & good_trial));
...
%% Store data   
    ...
    study_rt_data = vertcat(study_rt_data, ...
        ...
        { participant status study_cb list 'uninformed'  'SC'   median(study_rts(~informed & old_trial & old_resp & source_cor & good_trial)) }, ...
        { participant status study_cb list 'uninformed'  'SIDK' median(study_rts(~informed & old_trial & old_resp & source_miss & good_trial)) }, ...
        { participant status study_cb list 'uninformed'  'Miss' median(study_rts(~informed & old_trial & ~old_resp & good_trial)) } );
   ...
   %% Step 6 collect data for aggregate ROC data
    ...
    temp_luref = [];
     ...
     for i = 6:-1:1
            ...
            temp_targf(2,c) = sum( test_data.item_resp(old_trial & ~informed & good_trial) == i ) ;
...
writetable(memory_acc_data,fullfile(directories.analyses,'memory_acc_data_long.csv'));
